...
ACTION_URL      = 'https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date={train_time}&leftTicketDTO.from_station={from_city}&leftTicketDTO.to_station={to_city}&purpose_codes={ticket_type}'
#ACTION_URL      = 'https://kyfw.12306.cn/otn/lcxxcx/query?leftTicketDTO.train_date={train_time}&leftTicketDTO.from_station={from_city}&leftTicketDTO.to_station={to_city}&purpose_codes={ticket_type}'
...
#    print(url)
    ...
    print ('车次序号 \t     起始站 \t\t     出发站 \t\t     终点站   \t\t 发车时间 \t\t一等座 \t\t二等座')
    for r in ret['data']:
#	print r
        r = r['queryLeftNewDTO']
#	print r
#	print r['train_no'],r['start_station_name'],r['from_station_name'],r['to_station_name'],r['arrive_time'],r['zy_num'],r['ze_num']
#        if (not r['zy_num'].encode('utf-8').isdigit() 
#            and not r['ze_num'].encode('utf-8').isdigit()
#            or r['from_station_name'].encode('utf-8') != from_city):
#            continue
	m = re.match(r'^(\d+)(\w*)$', r['train_no']).groups()[1][:-2:1]
	if m == '':
		m=r['train_no'][6:10]
#	print m
        print (u'%s \t %15s \t %15s \t %15s \t %15s \t %10s \t %10s' %(
            m,
